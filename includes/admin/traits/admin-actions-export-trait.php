...
trait Admin_Actions_Export_Trait {
	...
	public function handle_rollback_download(): void {
		$this->assert_permissions();
		check_admin_referer( 'wa_acf_ptm_rollback_download', 'wa_acf_ptm_rollback_nonce' );
	...
	private function get_rollback_download_url( string $token ): string {
		return add_query_arg(
			array(
				'action'                  => 'wa_acf_ptm_download_rollback',
				'token'                   => $token,
				'wa_acf_ptm_rollback_nonce' => wp_create_nonce( 'wa_acf_ptm_rollback_download' ),
			),
	...
	private function stream_multi_export_zip( array $references, array $field_keys = array(), string $format = 'csv' ): void {
		...
		// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_readfile -- Streams a generated ZIP export to the browser.
		readfile( $temp_file );
		Temp_File_Service::delete( $temp_file );
		exit;
	}
