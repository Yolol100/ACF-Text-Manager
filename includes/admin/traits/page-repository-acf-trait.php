...
trait Page_Repository_ACF_Trait {
	...
	private function resolve_display_value( string $name, $post_id, string $key = '' ): string {
		...
		if ( '' === $this->field_values->normalize_mixed_for_presence_check( $value ) ) {
			$value = $this->get_raw_meta_fallback( $name, $post_id );
		}
	...
	private function get_raw_meta_fallback( string $name, $post_id ) {
		if ( is_numeric( $post_id ) ) {
			$post_meta_value = get_post_meta( (int) $post_id, $name, true );
			if ( '' !== $this->field_values->normalize_mixed_for_presence_check( $post_meta_value ) ) {
		...
		if ( is_string( $post_id ) && 0 === strpos( $post_id, 'term_' ) ) {
			...
			$term_meta_value = get_term_meta( $term_id, $name, true );
			if ( '' !== $this->field_values->normalize_mixed_for_presence_check( $term_meta_value ) ) {
				return $term_meta_value;
			}
			return '';
		}

		$option_value = get_option( (string) $post_id . '_' . $name, null );
		if ( '' !== $this->field_values->normalize_mixed_for_presence_check( $option_value ) ) {
			return $option_value;
		}

		if ( 'options' !== (string) $post_id ) {
			$default_option_value = get_option( 'options_' . $name, null );
			if ( '' !== $this->field_values->normalize_mixed_for_presence_check( $default_option_value ) ) {
