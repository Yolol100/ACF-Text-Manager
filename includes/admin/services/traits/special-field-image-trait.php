...
trait Special_Field_Image_Trait {
	...
	private function rename_attachment_file( int $attachment_id, string $new_value ): bool {
		if ( ! apply_filters( 'wa_acf_ptm_allow_media_file_rename', true, $attachment_id, $new_value ) ) {
			return false;
		}
		...
		if ( isset( $metadata['sizes'] ) && is_array( $metadata['sizes'] ) ) {
			foreach ( $metadata['sizes'] as $size_key => $size ) {
				...
				if ( $renamed_size_file !== $size_file ) {
					...
					if ( 0 !== strpos( $size_old_normalized, trailingslashit( $basedir ) ) || 0 !== strpos( $size_new_normalized, trailingslashit( $basedir ) ) ) {
						continue;
					}
					$size_renamed = false;
	...
	private function update_renamed_attachment_references( int $attachment_id, string $old_relative, string $new_relative, array $old_metadata, array $new_metadata ): array {
		...
		if ( ! apply_filters( 'wa_acf_ptm_update_media_rename_url_references', true, $attachment_id, $old_relative, $new_relative ) ) {
			return $counts;
		}
	...
	private function replace_urls_in_value_table( array $table_config, array $pairs ): int {
		...
		if ( ! $this->is_allowed_media_rename_reference_table( $table ) || '' === $id_column || '' === $value_column || ! preg_match( '/^[A-Za-z0-9_]+$/', $id_column ) || ! preg_match( '/^[A-Za-z0-9_]+$/', $value_column ) || ( '' !== $name_column && ! preg_match( '/^[A-Za-z0-9_]+$/', $name_column ) ) ) {
			return 0;
		}
		...
		foreach ( $pairs as $old_url => $new_url ) {
			...
			$rows = $wpdb->get_results(
				$wpdb->prepare( "SELECT {$id_column} AS row_id, {$value_column} AS stored_value{$select_name} FROM {$table} WHERE {$value_column} LIKE %s", $like ),
				ARRAY_A
			);
			foreach ( $rows as $row ) {
				...
				$unserialized = maybe_unserialize( $stored );
				$replaced = $this->replace_urls_in_value( $unserialized, $pairs );
				$new_stored = is_serialized( $stored ) ? maybe_serialize( $replaced ) : (string) $replaced;
	...
	private function is_allowed_media_rename_reference_table( string $table ): bool {
		global $wpdb;
		...
		$allowed_tables = apply_filters(
			'wa_acf_ptm_allowed_media_rename_reference_tables',
			array( $wpdb->postmeta, $wpdb->termmeta, $wpdb->options )
		);

		if ( ! is_array( $allowed_tables ) ) {
			return false;
		}

		$allowed_tables = array_map( 'strval', $allowed_tables );
