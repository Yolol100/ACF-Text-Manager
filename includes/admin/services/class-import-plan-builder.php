...
final class Import_Plan_Builder {
	...
	public function build( string $token, array $target, array $dataset, array $mapping, string $file_name ): array {
		...
		foreach ( $rows as $row ) {
			...
			if ( $target_mismatch ) {
				...
				$preview_rows[] = array(
					...
					'field_name'    => (string) $field['name'],
					'field_type'    => (string) $field['type'],
					'current_value' => $this->preview_value( $current_value ),
					'new_value'     => $this->preview_value( $new_value ),
					'reason'        => $reason,
					'warning'       => $warning,
				);
				if ( '' !== $warning ) {
			...
			$preview_rows[] = array(
				'row_index'     => (int) $row['row_index'],
				'action'        => 'skip',
				'field_label'   => ! empty( $row['field_label'] ) ? (string) $row['field_label'] : (string) $row['field_name'],
				'field_name'    => (string) $row['field_name'],
				'field_type'    => (string) $row['field_type'],
				'current_value' => '',
				'new_value'     => $this->preview_value( (string) $row['value'] ),
				'reason'        => $reason,
				'warning'       => '',
			);
			$skips++;
		...
		return array(
			...
			'preview_rows'      => array_slice( $preview_rows, 0, 25 ),
			'summary'           => array(
	...
	private function preview_value( $value ): string {
		if ( is_array( $value ) || is_object( $value ) ) {
			$encoded = wp_json_encode( $value, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES );
			return is_string( $encoded ) ? $this->trim_preview_text( $encoded ) : '';
		}
		...
		return $this->trim_preview_text( is_scalar( $value ) ? (string) $value : '' );
	}

	private function trim_preview_text( string $text ): string {
